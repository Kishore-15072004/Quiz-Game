...
tryAgainBtn.onclick=()=>{
    ...
    selectedQuestions = [];
    let tempQuestions = [...questions]; // Create a copy of the original questions array
    for(let i = 0; i < 10; i++){
        let index = Math.floor(Math.random() * tempQuestions.length);
        selectedQuestions.push(tempQuestions[index]);
        tempQuestions.splice(index, 1); // This will prevent the same question from being selected more than once
    }
...
function showQuestions(index) {
    ...
    let optionTag =`<div class="option"><span>${selectedQuestions[index].options[0]}</span></div>
